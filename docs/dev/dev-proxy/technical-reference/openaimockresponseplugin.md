...
The `OpenAIMockResponsePlugin` plugin supports the following OpenAI API endpoints:

- **Chat Completions** (`/v1/chat/completions`) — uses the `messages` array and returns `choices` with `prompt_tokens`/`completion_tokens` usage fields
- **Responses API** (`/v1/responses`) — uses the `input` array and returns `output` with `input_tokens`/`output_tokens` usage fields

The plugin doesn't support other OpenAI API endpoints.
