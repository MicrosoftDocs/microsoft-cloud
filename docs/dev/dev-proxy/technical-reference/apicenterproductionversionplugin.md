...
<!-- INTENT: Check if APIs are production versions in Azure API Center -->
<!-- PLUGIN-TYPE: Reporting -->
<!-- WORKS-WITH: ApiCenterOnboardingPlugin, ApiCenterMinimalPermissionsPlugin, MarkdownReporter -->
<!-- USE-WHEN: Ensuring apps use production-ready API versions -->
...
```json
{
  ...
  "apiCenterProductionVersionPlugin": {
    "$schema": "https://raw.githubusercontent.com/dotnet/dev-proxy/main/schemas/v2.0.0/apicenterproductionversionplugin.schema.json",
...
In this example, the `ApiCenterProductionVersionPlugin` plugin sets `subscriptionId`, `resourceGroupName`, `serviceName`, and `workspace` properties to the values of the `AZURE_SUBSCRIPTION_ID`, `AZURE_RESOURCE_GROUP_NAME`, `AZURE_APIC_INSTANCE_NAME`, and `AZURE_APIC_WORKSPACE_NAME` environment variables, respectively.
