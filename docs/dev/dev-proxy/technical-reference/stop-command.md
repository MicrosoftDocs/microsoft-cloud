...
Dev Proxy uses the current user's instance credential to authenticate the graceful stop request. If the graceful stop fails (for example, because the API port or credential is unavailable), use the `--force` option to forcefully terminate the process.
