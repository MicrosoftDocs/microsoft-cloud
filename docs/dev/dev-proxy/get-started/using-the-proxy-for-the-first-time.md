...
You learn how to use the Dev Proxy for the first time.
...
1. **Start the proxy**. Open a terminal. Enter `devproxy` and press <kbd>Enter</kbd>.
...
The proxy can issue error responses for any API. To add your own API, change the contents of the `devproxyrc.json` file.

1. Open the `devproxy` folder to find the `devproxyrc.json` file.
1. Open the `devproxyrc.json` in a text editor and locate the `urlsToWatch` array in the root object (not in the plugins array).
