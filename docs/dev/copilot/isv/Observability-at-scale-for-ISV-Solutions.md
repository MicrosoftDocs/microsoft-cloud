...
When deploying a Microsoft Cloud application, there are several [landing zones and deployment models](/azure/cloud-adoption-framework/ready/landing-zone/isv-landing-zone) ISVs can choose from. The ways that you practice observability varies depending on the deployment pattern and platforms you choose. For instance, it's typically easier for an ISV to monitor their own SaaS platform than to gain visibility into software that a customer deploys themselves.
...
- [Azure Monitor overview - Azure Monitor | Microsoft Learn](/azure/azure-monitor/overview) - In-depth guidance on Azure monitor and its many capabilities.
