...
| WarningThresholdPercent | The percentage of use that's when exceed will start returning rate limiting response headers     |             80             |
...
| WhenLimitExceeded       | The behaviour the plugin should use when limit is exceeded. Use `Throttle` or `Custom`.          |         `Throttle`         |
